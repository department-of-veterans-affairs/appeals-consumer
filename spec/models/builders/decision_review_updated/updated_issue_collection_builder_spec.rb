...
RSpec.describe Builders::DecisionReviewUpdated::UpdatedIssueCollectionBuilder, type: :model do
  ...
  describe "#updated_issues" do
    before do
      ...
      message_payload["decision_review_issues_created"].push(
        base_decision_review_issue.merge(
          ...
          "contention_action" => contention_updated,
          "reason_for_contention_action" => text_changed
        ),
        base_decision_review_issue.merge(
          "contention_id" => 123_457,
          "contention_action" => no_contention_action,
          "reason_for_contention_action" => text_changed
        )
      )
    ...
    context "when decision review updated issues are present" do
      ...
      it "only returns issues with a reason_for_contention_action of 'PRIOR_DECISION_TEXT_CHANGED'" do
        subject.updated_issues.each do |issue|
          expect(issue.reason_for_contention_action).to eq(text_changed)
        end
      end

      it "only returns issues with a contention_action of 'NONE' OR 'UPDATE_CONTENTION" do
        subject.updated_issues.each do |issue|
          expect(issue.contention_action).to eq(no_contention_action).or eq(contention_updated)
        end
      end
