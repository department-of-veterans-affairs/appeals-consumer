...
describe Events::DecisionReviewCreatedEvent, type: :model do
  let(:event) { create(:decision_review_created_event) }

  describe "#process!" do
    let(:dto_builder_instance) { instance_double("Builders::DecisionReviewCreated::DtoBuilder") }
    let(:caseflow_response) { instance_double("Response", code: 201, message: "Some message") }
    subject { event.process! }

    before do
      allow(Builders::DecisionReviewCreated::DtoBuilder).to receive(:new).with(event).and_return(dto_builder_instance)
