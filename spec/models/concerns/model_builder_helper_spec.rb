...
describe DecisionReview::ModelBuilderHelper do
  let(:decision_review_created_dummy) { DummyClass.new }
  let(:decision_review_updated_dummy) { DummyClass.new }
  ...
  let(:update_time) { Time.now.utc.to_s }
  ...
  before do
    ...
    decision_review_created_dummy.decision_review_model = @decision_review_created_double

    @decision_review_updated_double = instance_double(
    ...
    decision_review_updated_dummy.decision_review_model = @decision_review_updated_double
  end

  describe "#fetch_veteran_bis_record" do
    subject { decision_review_created_dummy.fetch_veteran_bis_record }
    context "when decision_review_model is present" do
      context "when the BIS record is found" do
        it "fetches the veteran BIS record successfully" do
          ...
          expect(decision_review_created_dummy.bis_synced_at).not_to be_nil
        end
      end
      ...
      context "when the BIS record is not found" do
        let(:msg) do
          ...
          " #{decision_review_created_dummy.decision_review_model.file_number}"
        end
  ...
  describe "#fetch_person_bis_record" do
    subject { decision_review_created_dummy.fetch_person_bis_record }
    context "when decision_review_model is present" do
      context "when the BIS record is found" do
        it "fetches the veteran BIS record successfully" do
      ...
      context "when the BIS record is not found" do
        let(:msg) do
          "BIS Person: Person record not found for DecisionReviewCreated claimant_participant_id:"\
            " #{decision_review_created_dummy.decision_review_model.claimant_participant_id}"
        end
        let!(:event_audit_without_note) { create(:event_audit, event: event, status: :in_progress) }
  ...
  describe "#fetch_limited_poa" do
    context "when decision_review_model is present" do
      it "fetches the limited POA successfully" do
        ...
        expect(decision_review_created_dummy.fetch_limited_poa).to eq("POA info")
      end
      ...
      it "returns nil if the limited POA is not found" do
        ...
        expect(decision_review_created_dummy.fetch_limited_poa).to be_nil
      end
      ...
      context "an error is thrown" do
        ...
        it "logs the error" do
          expect(Rails.logger).to receive(:error).with(/#{timeout_msg}/)
          decision_review_created_dummy.fetch_limited_poa
        end
    ...
    context "when decision_review_model is nil" do
      it "returns nil" do
        decision_review_created_dummy.decision_review_model = nil
        expect(decision_review_created_dummy.fetch_limited_poa).to be_nil
      end
    end
  end
  ...
  describe "#fetch_bis_rating_profiles" do
    subject { decision_review_created_dummy.fetch_bis_rating_profiles }
    ...
    context "when decision_review_model, earliest_issue_profile_date, and"\
      ...
      before do
        decision_review_created_dummy.earliest_issue_profile_date = earliest_date.to_date
        decision_review_created_dummy.latest_issue_profile_date_plus_one_day = latest_date.to_date + 1
      end
    ...
    context "when decision_review_model, earliest_issue_profile_date, or latest_issue_profile_date_plus_one_day"\
      ...
      context "when earliest_issue_profile_date is not present" do
        it "returns nil" do
          decision_review_created_dummy.earliest_issue_profile_date = nil
          expect(subject).to be_nil
        end
      end
      ...
      context "when latest_issue_profile_date_plus_one_day is not present" do
        it "returns nil" do
          decision_review_created_dummy.latest_issue_profile_date_plus_one_day = nil
          expect(subject).to be_nil
        end
      end
    end
  ...
  describe "#convert_to_date_logical_type(value)" do
    ...
    context "when the value is not nil" do
      it "returns the value converted to date logical type" do
        claim_received_date = decision_review_created_dummy.decision_review_model.claim_received_date
        expect(decision_review_created_dummy.convert_to_date_logical_type(claim_received_date).class).to eq(Integer)
      end
    end
  end
  ...
  describe "#claim_creation_time_converted_to_timestamp_ms" do
    context "when the decision_review_model is nil" do
      it "returns nil" do
        decision_review_created_dummy.decision_review_model = nil
        expect(decision_review_created_dummy.claim_creation_time_converted_to_timestamp_ms).to be_nil
      end
    end
    ...
    context "when decision_review_created_is_not_nil" do
      ...
      context "when claim_creation_time is not nil" do
        it "returns claim_creation_time converted to timestamp ms" do
          expect(decision_review_created_dummy.claim_creation_time_converted_to_timestamp_ms).not_to be_nil
        end
      end
    end
  end
  ...
  describe "#update_time_converted_to_timestamp_ms" do
    context "when the decision_review_model is nil" do
      it "returns nil" do
        decision_review_updated_dummy.decision_review_model = nil
        expect(decision_review_updated_dummy.update_time_converted_to_timestamp_ms).to be_nil
      end
    end
    ...
    context "when decision_review_model is NOT nil" do
      ...
      context "when updated_time is NOT nil" do
        it "returns updated_time converted to timestamp ms" do
          expect(decision_review_updated_dummy.update_time_converted_to_timestamp_ms).not_to be_nil
        end
      end
    end
  end
  ...
  describe "#downcase_bis_rating_profiles_response_text" do
    context "when @bis_rating_profiles_record is nil" do
      it "returns nil" do
        expect(decision_review_created_dummy.send(:downcase_bis_rating_profiles_response_text)).to eq(nil)
      end
    end
    ...
    context "when @bis_rating_profiles is not nil" do
      context ":response_text key exists" do
        ...
        before do
          decision_review_created_dummy.instance_variable_set(:@bis_rating_profiles_record, hash_with_response_text)
        end

        it "returns the value of :response_text and converts to downcase" do
          expect(decision_review_created_dummy.send(:downcase_bis_rating_profiles_response_text)).to eq("no data found")
        end
      end
      ...
      context ":response_text key does not exist" do
        ...
        before do
          decision_review_created_dummy.instance_variable_set(:@bis_rating_profiles_record, hash_without_response_text)
        end
  ...
  describe "#log_msg_and_update_current_event_audit_notes!(msg, error: false)" do
    ...
    context "when error: true is not passed as an argument" do
      it "logs the message as info" do
        allow(Rails.logger).to receive(:info)
        decision_review_created_dummy.send(:log_msg_and_update_current_event_audit_notes!, msg)
    ...
    context "when error: true is passed as an argument" do
      it "logs the message as error" do
        ...
        decision_review_created_dummy.send(:log_msg_and_update_current_event_audit_notes!, msg, error: true)
  ...
  describe "#log_msg(msg, error)" do
    ...
    context "when error: false is passed as an argument" do
      it "logs the message as info" do
        allow(Rails.logger).to receive(:info)
        decision_review_created_dummy.send(:log_msg, msg, false)
        expect(Rails.logger).to have_received(:info).with(/#{msg}/)
      end
    end
    ...
    context "when error: true is passed as an argument" do
      it "logs the message as error" do
        allow(Rails.logger).to receive(:error)
        decision_review_created_dummy.send(:log_msg, msg, true)
        expect(Rails.logger).to have_received(:error).with(/#{msg}/)
      end
    end
  end
  ...
  describe "logger" do
    ...
    it "initializes a new instance of LoggerService" do
      expect(LoggerService).to receive(:new).with(decision_review_created_dummy.class.name)
      decision_review_created_dummy.send(:logger)
    end
  end
  ...
  describe "#log_info(msg)" do
    ...
    it "logs the message as info" do
      expect(Rails.logger).to receive(:info).with(/#{msg}/)
      decision_review_created_dummy.send(:log_info, msg)
    end
  end
  ...
  describe "#log_error(msg)" do
    ...
    it "logs the message as error" do
      ...
      decision_review_created_dummy.send(:log_error, msg)
    end
  end
  ...
  describe "#update_event_audit_notes!(msg)" do
    ...
    context "when the event doesn't have any event_audits with 'in_progress' status" do
      it "returns nil" do
        expect(decision_review_created_dummy.send(:update_event_audit_notes!, msg)).to eq nil
      end
    ...
    context "when the event has one event_audit with 'in_progress' status" do
      ...
      it "finds the event_audit and updates the notes column with the custom note" do
        decision_review_created_dummy.send(:update_event_audit_notes!, msg)
        expect(event_audit_without_note.reload.notes).to eq("Note #{Time.zone.now}: #{msg}")
      end
    end
      ...
      context "when the event_audit already has a note" do
        ...
        context "one note" do
          ...
          it "concatenates the previous note and adds default message between notes" do
            ...
            decision_review_created_dummy.send(:update_event_audit_notes!, msg_2)
            expect(event_audit_with_note.reload.notes)
        ...
        context "two notes" do
          ...
          it "concatenates the previous notes and adds default message between notes" do
            ...
            expect(event_audit_with_note.reload.notes)
              .to eq("Note #{Time.zone.now}: #{msg} - Note #{Time.zone.now}: #{msg_2}")
            decision_review_created_dummy.send(:update_event_audit_notes!, msg_3)
            expect(event_audit_with_note.reload.notes)
              .to eq("Note #{Time.zone.now}: #{msg} - Note #{Time.zone.now}: #{msg_2} -"\
  ...
  describe "#event_audit_concatenated_notes(last_event_audit, msg)" do
    ...
    context "when the last event_audit's notes is nil" do
      ...
      it "returns the custom message" do
        expect(decision_review_created_dummy.send(:event_audit_concatenated_notes, last_event_audit, msg))
          .to eq("Note #{Time.zone.now}: #{msg}")
      end
    end
    ...
    context "when the last event_audit already has a note" do
      ...
      it "concatenates the previous notes value with the custom note" do
        expect(decision_review_created_dummy.send(:event_audit_concatenated_notes, last_event_audit, msg_2))
