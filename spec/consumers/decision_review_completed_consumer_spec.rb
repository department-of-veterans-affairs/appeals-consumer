...
describe DecisionReviewCompletedConsumer do
  ...
  let(:event_type) { "Events::DecisionReviewCompletedEvent" }
  let(:decision_review_completed_extra_details) do
    {
  ...
  describe "#consume" do
    ...
    let(:new_record) { true }
    consumer_name = /\[DecisionReviewCompletedConsumer\]/

    before do |example|
      unless example.metadata[:skip_before]
        allow(consumer).to receive(:messages).and_return([message])
        allow(Events::DecisionReviewCompletedEvent)
    ...
    context "when event is a new record" do
      it "saves the event, enqueues into DecisionReviewCompletedEventProcessingJob, and calls MetricsService.record" do
        expect(event).to receive(:save)
        expect(DecisionReviewCompletedEventProcessingJob).to receive(:perform_later).with(event)
