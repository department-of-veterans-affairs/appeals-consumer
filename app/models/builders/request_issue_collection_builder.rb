...
class Builders::RequestIssueCollectionBuilder
  ...
  def handle_no_issues_after_removing_contested
    fail AppealsConsumer::Error::RequestIssueCollectionBuildError, "Failed building from"\
      " Builders::RequestIssueCollectionBuilder for DecisionReviewCreated Claim ID:"\
  ...
  def build_request_issue(issue, index)
    begin
      ...
      Builders::RequestIssueBuilder.build(issue, @decision_review_created)
    rescue StandardError => error
      message = "Failed building from Builders::RequestIssueCollectionBuilder for DecisionReviewCreated"\
        " Claim ID: #{@decision_review_created.claim_id} #{issue_identifier_message(issue, index)} - #{error.message}"
